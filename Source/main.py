...
def get_latest_release_info(owner, repo, app_filename):
...
def update_and_restart(download_zip_url):
    ...
    if os.path.basename(current_app_path).lower()!=APP_FILENAME.lower():
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                subprocess.Popen([app_updater_path, current_app_path, downloaded_app_path_new],
                                 creationflags=subprocess.DETACHED_PROCESS|subprocess.CREATE_NEW_PROCESS_GROUP) # Отключает процесс от родительского (окна консоли) на Windows
...
def check_update():
    ...
    for _ in range(5):
        latest_tag, download_zip_url = get_latest_release_info(GITHUB_OWNER, GITHUB_REPO, ZIP_FILENAME)
    ...
    if latest_tag and download_zip_url:
        ...
        try:
            ...
            if latest_v > current_v:
                ...
                pass
                # print("Проверка обновления: у вас уже установлена последняя версия.")
            else:
                pass
...
def check_new_archives_rclone(remote_folder, review_period, depth_folder):
    ...
    for file_arc in remote_file_list:
        ...
        file_name = os.path.splitext(file_arc["Name"])[0]
        file_name = file_name.replace(time_stamp, "").strip()
...
def check_cmd_param():
    param_str = "userlist,systray,sendfile,userinfo"
...
def test_rclone(remote_folder, min_empty_space):
    ...
    if free_gb < min_empty_space:
        print_error("На диске в облаке мало свободного места: " + str(free_gb) + " гб (занято "+str(percent)+"%), всего свободного места: " + str(total_gb) + " гб")
    print_info(" + в облаке свободного места: " + str(free_gb) + " гб (занято "+str(percent)+"%), всего свободного места: " + str(total_gb) + " гб")
